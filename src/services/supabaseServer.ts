...
export const syncSubscriptionFromSupabase = async (
  ...
  try {
    ...
    const res = await fetch(
      `${baseUrl}/api/subscriptions/status?telegramUserId=${encodeURIComponent(tgId)}&username=${encodeURIComponent(tgUser)}&bankId=${encodeURIComponent(bankId)}`,
      {
        headers: {
    ...
    if (!res.ok) return getSubscriptionStatus(tgId, bankId);
    const data = await res.json();
    if (data.success) {
      const subs = getStoredSubscriptions();
      const userSubKey = `${tgId}_${bankId}`;
      ...
      const subObj: Subscription = {
        ...
        bankId,
        status,
        plan: data.subscription?.plan || subs[userSubKey]?.plan || 'U JO TAJNEED Pass',
  ...
  const user = getCurrentUser(telegramUserId);
  return getSubscriptionStatus(user.telegramId, bankId);
};
